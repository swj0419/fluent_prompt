          ...
          do
            ...
            do
              ...
              do
                ...
                # large batch size
                MY_EXP_DIR="/private/home/swj0419/i-am-a-dog/openprompt-clone/swj_logging_dir/new2/amazon/gpt2-large/headprompt{$LENGTH}_ppl_loss_lambda{$ppl_loss_lambda}_ppl_loss_example_lambda{$ppl_loss_example_lambda}_task_loss_lambda{$task_loss_lambda}_entropy_loss_lambda{$entropy_loss_lambda}_LR{$LR}_NSTART{$NSTART}_NEND{$NEND}_SEED{$SEED}"
                # mkdir -p $MY_EXP_DIR
