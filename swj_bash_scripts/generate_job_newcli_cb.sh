...
do
  for NSTART in 1.0
  do
    for LR in 10.0 3.0 1.0
    do
      for ppl_loss_lambda in 0
      do
        for ppl_loss_example_lambda in 0.0 0.03 0.2 0.5
        do
          for task_loss_lambda in 0
          do
            for entropy_loss_lambda in 0
            do
              for SEED in $(seq 15 1 25)
              do
                ...
                mkdir -p "/private/home/swj0419/i-am-a-dog/openprompt-clone/swj_logging_dir/new/cb"
                # large batch size
                MY_EXP_DIR="/private/home/swj0419/i-am-a-dog/openprompt-clone/swj_logging_dir/new/cb/gpt2-large/headprompt{$LENGTH}_ppl_loss_lambda{$ppl_loss_lambda}_ppl_loss_example_lambda{$ppl_loss_example_lambda}_task_loss_lambda{$task_loss_lambda}_entropy_loss_lambda{$entropy_loss_lambda}_LR{$LR}_NSTART{$NSTART}_NEND{$NEND}_SEED{$SEED}"
                echo "my_experiments/new_cli.py \
                    --config_yaml my_experiments/cb_gpt2style_headprompt.yaml \
                    ...
                    --my_global_seed $SEED \
                    --prompt_num_tokens $LENGTH" >> hrpt_cb.txt
              done
            done
          done
        done
