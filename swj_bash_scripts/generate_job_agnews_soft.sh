  ...
  do
    ...
    do
      ...
      do
        for SEED in $(seq 0 1 4) # $(seq 0 1 4)
        do
          mkdir -p "/private/home/swj0419/i-am-a-dog/openprompt-clone/swj_logging_dir/amazon/gpt2-large_soft"
          MY_EXP_DIR="/private/home/swj0419/i-am-a-dog/openprompt-clone/swj_logging_dir/agnews2/gpt2-large_soft/headprompt{$LENGTH}_LAMBDA{$LAMBDA}_LR{$LR}_NSTART{$NSTART}_NEND{$NEND}_SEED{$SEED}"
          ...
          echo "my_experiments/soft_cli.py \
              ...
              --my_global_seed $SEED \
              --prompt_num_tokens $LENGTH" >> hrpt_agnews_soft.txt
        done
      done
    done
  done
