...
do
  for NSTART in 1.0
  do
    for LR in 3.0 1.0 0.3
    do
      for ppl_loss_lambda in 0
      do
        for ppl_loss_example_lambda in 0.0 0.05 0.2 0.5 0.8 0.95 1.0
        do
          ...
          do
            ...
            do
              ...
              do
                ...
                # large batch size
                MY_EXP_DIR="/private/home/swj0419/i-am-a-dog/openprompt-clone/swj_logging_dir/new/sst2/gpt2-large/headprompt{$LENGTH}_ppl_loss_lambda{$ppl_loss_lambda}_ppl_loss_example_lambda{$ppl_loss_example_lambda}_task_loss_lambda{$task_loss_lambda}_entropy_loss_lambda{$entropy_loss_lambda}_LR{$LR}_NSTART{$NSTART}_NEND{$NEND}_SEED{$SEED}"
