...
do
  for NSTART in 1.0 0.1
  do
    for LR in 30.0 10.0 3.0 1.0 0.3
    do
      for LAMBDA in 0.003 0.01 0.03 0.1 0.3
      do
        for SEED in $(seq 0 1 29) # $(seq 0 1 4)
        do
          mkdir -p "/private/home/swj0419/i-am-a-dog/openprompt-clone/swj_logging_dir/imdb/gpt2-large"
          MY_EXP_DIR="/private/home/swj0419/i-am-a-dog/openprompt-clone/swj_logging_dir/imdb/gpt2-large/headprompt{$LENGTH}_LAMBDA{$LAMBDA}_LR{$LR}_NSTART{$NSTART}_NEND{$NEND}_SEED{$SEED}"

          echo "my_experiments/cli.py \
              --config_yaml my_experiments/imdb_gpt2style_headprompt.yaml \
              ...
              --my_global_seed $SEED \
              --prompt_num_tokens $LENGTH" >> hrpt_imdb.txt
