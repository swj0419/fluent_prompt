...
class MyClassificationRunner(BaseRunner):
        ...
        with tqdm(total=self.steps_per_epoch, desc=f"train epoch: {epoch}") as pbar:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if (batch_idx + 1) % self.config.train.gradient_accumulation_steps == 0:
                    ...
                    #     self.model.prompt_model.template.soft_embeds.add_(noise) # removed in soft_cli
                    ...
                    #     with torch.no_grad():
                    #         self.model.prompt_model.template.soft_embeds.copy_(proj_embed_mat.clone().detach()) # removed in soft_cli
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = task_loss
        ppl_loss = task_loss # HACK, for soft_cli
