...
class MyClassificationRunner(BaseRunner):
    ...
    def observe_prompt_tokens(self):
        ...
        logger.info(f"real prompt tokens: {self.model.prompt_model.template.convert_soft_embeds2_token(proj_embed_mat)}")
    ...
    def training_epoch(self, epoch):
        ...
        with tqdm(total=self.steps_per_epoch, desc=f"train epoch: {epoch}") as pbar:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                sum_loss += loss.item()
                sum_task_loss += task_loss.item()
                sum_ppl_loss += ppl_loss.item()
                loss.backward()
                if (batch_idx + 1) % self.config.train.gradient_accumulation_steps == 0:
                    ...
                    # observe for each step (temporary)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ppl loss
        proj_prob_mat, _proj_embed_mat = self.model.prompt_model.template.make_projected_embeds(projection_temperature=self.config.objective_prompt_labels_temp)
        real_plm_logits = self.model.prompt_model.plm_logits # Han: can only be used after a model forward pass, size: (batch_size, max_seq_len, vocab_size)
        plm_logits_at_soft_prompts = real_plm_logits[:, :proj_prob_mat.size(0),: ] # size: (batch_size, len_of_soft_prompt, vocab_size); Han: highly depend on the implementation of soft_template.py

        ppl_loss = self.ppl_loss_function(plm_logits_at_soft_prompts, proj_prob_mat)
