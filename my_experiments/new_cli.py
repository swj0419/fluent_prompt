...
class MyClassificationRunner(BaseRunner):
        ...
        with tqdm(total=self.steps_per_epoch, desc=f"train epoch: {epoch}") as pbar:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if (batch_idx + 1) % self.config.train.gradient_accumulation_steps == 0:
                    ...
                    logger.info(f"sum ppl loss: {sum_ppl_loss}")
    ...
    def training_step(self, batch, batch_idx):
        ...
        # [NEW] add a new entropy loss w.r.t. the batch's output distribution
        batch_probs = torch.nn.functional.softmax(logits, dim=-1) # dim: BS x K_labels
        agg_probs = batch_probs.mean(dim=-2) # mean across batch, for each label confidence
        entropy_loss = torch.sum(agg_probs * torch.log2(agg_probs))
        ...
        # [NEW] add a new ppl loss w.r.t. the training input
        lm_logits = self.model.prompt_model.plm.lm_head(real_plm_last_h[:, soft_prompts.size(0):, :].contiguous()) # works for GPT2
        shift_logits = lm_logits[..., :-1, :].contiguous() # dimension should match the line below
        shift_labels = batch['input_ids_copy'][..., 1:].contiguous() # Han: probably will fail, assuming dimension excluding soft tokens
        shift_labels[shift_labels == self.model.tokenizer.pad_token_id] = -100 # <pad> has an id of 50257 in our tokenizer
        loss_fct = torch.nn.CrossEntropyLoss()
        new_ppl_loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
        # breakpoint()
        # hack_choice = 'new_2' # select from: cur, new_1, new_2
        #
        # if hack_choice == 'cur':
        #     # OUR CURRENT VARIANT
        #     loss = (1 - self.config.ppl_loss_lambda) * task_loss + self.config.ppl_loss_lambda * ppl_loss
        # elif hack_choice == 'new_1':
        #     # NEW VARIANT 1 (incorporating full sequence ppl)
        #     loss = (1 - self.config.ppl_loss_lambda) * task_loss + self.config.ppl_loss_lambda * ppl_loss + self.config.ppl_loss_lambda * new_ppl_loss
        # elif hack_choice == 'new_2':
        #     # NEW VARIANT 2 (incorporating entropy loss, but no task loss, also no full sequence loss)
        #     # Han: for this loss, we probably need a larger batch size (currently 4?)
        #     loss = (1 - self.config.ppl_loss_lambda) * entropy_loss + self.config.ppl_loss_lambda * ppl_loss
        loss = self.config.task_loss_lambda * task_loss + self.config.ppl_loss_lambda * ppl_loss \
               + self.config.ppl_loss_example_lambda * new_ppl_loss + self.config.entropy_loss_lambda * entropy_loss
        return loss, task_loss, new_ppl_loss
...
def my_get_config():
    ...
    parser.add_argument("--ppl_loss_example_lambda", type=float, default=0.5, help='')
    parser.add_argument("--task_loss_lambda", type=float, default=0.5, help='')
    parser.add_argument("--entropy_loss_lambda", type=float, default=0.5, help='')
    ...
    config.soft_template.prompt_text = args.prompt_text
    ...
    config.ppl_loss_example_lambda = args.ppl_loss_example_lambda
    config.task_loss_lambda = args.task_loss_lambda
    config.entropy_loss_lambda = args.entropy_loss_lambda
