...
class TokenizerWrapper:
    ...
    def add_special_tokens(self, encoder_inputs):
        # Han: a bit hacky to resolve bug
        encoder_inputs['input_ids'] = self.tokenizer.build_inputs_with_special_tokens(encoder_inputs['input_ids'])
        special_tokens_mask = np.array([1 if tok in self.tokenizer.all_special_ids else 0 for tok in encoder_inputs['input_ids']])
        with_special_tokens = np.array(self.tokenizer.build_inputs_with_special_tokens(encoder_inputs['loss_ids']))
        encoder_inputs['loss_ids'] = ((1-special_tokens_mask) * with_special_tokens).tolist()
        return encoder_inputs

        # for key in encoder_inputs:
        #     breakpoint() # Han: debug
        #     if key == "input_ids":
        ...
        #             raise ValueError("disabled") # Han: we don't need this in our experiment
        #             encoder_inputs[key] =  ((1-special_tokens_mask) * with_special_tokens).tolist() # use 0 as special
        #         elif key in ["loss_ids"]: # Han: we need this
        #             encoder_inputs[key]
        #         else:
        #             raise ValueError("disabled") # Han: we don't need this in our experiment
