...
class SuperglueBoolQProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        if split == "valid" or split == "dev":
            # raise FileNotFoundError("disabled for now") # Han: move the original validation set as the test set
            split = "validation"
        elif split == "test":
            split = "validation" # Han: move the original validation set as the test set; not okay for comparing with SOTA models, but should be fine for our analysis purpose
        try:
