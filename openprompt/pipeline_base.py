...
class PromptModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.plm_logits = None
        self.plm_last_hidden_states = None
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        self.plm_logits = outputs['logits'].clone()
        if 'hidden_states' in outputs:
            self.plm_last_hidden_states = outputs['hidden_states'][-1].clone() # optional: check openprompt/plms/seq2seq.py:T5LMTokenizerWrapper
        else:
            raise ValueError("decided to change framework: add a separate ppl module instead of using a single LM for task")
